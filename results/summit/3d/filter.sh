...
g=$(grep "^Vectors = $v:" $f | awk '{print substr($10,1,8)}')
...
aa=$(grep "^Vectors = $v:" $f | awk '{print $7}')
rep=$(grep "^Vectors = $v:" $f | awk '{print $13}')
...
g=$(printf '%12s' $g)
echo "$np $vv $min $avg $max $g -> $mins $avgs $maxs ($a $rep)" >> $ofile
done 
done 
echo "" >> $ofile
echo "---------------------------------------------------------------------------------------------------------------" >> $ofile
